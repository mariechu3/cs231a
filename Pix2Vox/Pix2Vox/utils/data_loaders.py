...
class ShapeNetDataLoader:
    def __init__(self, cfg):
        ...
        print("file path",cfg.DATASETS.SHAPENET.TAXONOMY_FILE_PATH )
...
class Pix3dDataset(torch.utils.data.dataset.Dataset):
    ...
    def get_datum(self, idx):
        ...
        with open(volume_path, 'rb') as f:
            volume = scipy.io.loadmat(volume_path)
            print("keys:", volume.keys())
            volume = volume['voxel'].astype(np.float32)
            print("before interpolate", volume.shape)
            volume = torch.nn.functional.interpolate(torch.from_numpy(volume), size = (32,32,32))
            volume = volume.numpy()
            print(volume.shape)
        print("got here sucessfully----------------------------------------------------")
...
class Pix3dDataLoader:
    ...
    def get_files_of_taxonomy(self, taxonomy_name, samples):
        ...
        for sample_idx, sample_name in enumerate(samples):
            ...
            volume_file_path = self.volume_path_template % (taxonomy_name, model_name, model_name_parts[3])
            if not os.path.exists(volume_file_path):
                print('[WARN] %s Ignore sample %s/%s since volume file not exists.' %
                      (dt.now(), taxonomy_name, sample_name))
                continue
        ...
        print("files of taxonomy-----------------------", files_of_taxonomy)
        return files_of_taxonomy
