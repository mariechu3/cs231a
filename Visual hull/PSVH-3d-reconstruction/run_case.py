...
def examples(model='syn'):
    ...
    weight_path = os.path.join('models','%s_model'%model,'model.ckpt')
    ...
    files = [file for file in os.listdir(os.path.join(source_path,"shapeNet"))]

    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        saver.restore(sess,weight_path)
        for file in files:
            filepath = os.path.join(source_path,'shapeNet',file)
            ...
            #added for shapenet
            v_before = np.transpose(v_before, (2,0,1))
            v_after = np.transpose(v_after, (2,0,1))

            v_before = np.flip(v_before, axis=0)
            v_before = np.flip(v_before, axis=2)
            v_after = np.flip(v_after, axis=0)
            v_after = np.flip(v_after, axis=2)

            # for pix3d
            # v_before = np.flip(v_before, axis=1)
            # v_after = np.flip(v_after, axis=1)
            


            np.save('shapeNet/%s_before'%file[:-4], v_before)
            np.save('shapeNet/%s_after'%file[:-4], v_after)
